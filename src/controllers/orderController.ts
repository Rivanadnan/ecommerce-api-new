...
export const getOrderBySessionId = async (req: Request, res: Response) => {
  ...
  try {
    ...
    `;
    const [rows] = await db.query<IOrder[]>(sql, [session_id]);
    rows && rows.length > 0
      ? res.json(formatOrderDetails(rows))
      : res.status(404).json({ message: "Order not found" });
  } catch (error) {
...
export const updateOrderBySessionId = async (req: Request, res: Response) => {
  const session_id: string = req.params.session_id;
  const { payment_status, order_status }: IOrder = req.body;
...
export const createOrder = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const totalPrice = req.body.order_items.reduce(
      (total: number, item: IOrderItem) => total + item.quantity * item.unit_price,
      0
    );
